...
function generateId(): string {
  return `triage-${Date.now()}-${Math.random().toString(36).substring(2, 8)}`;
}
        ...
        <div className="lg:col-span-2 space-y-4">
              ...
              {history.length === 0 ? (
                <div className="text-center py-8">
                  <p className="text-xs text-emerald-700">
                    <span className="text-emerald-600">$</span> no triages yet
                  </p>
                  <p className="text-[10px] text-emerald-800 mt-1">run a triage to see history</p>
                </div>
              ) : (
                  ...
                  {history.map((entry) => (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-[10px] text-emerald-600 font-mono truncate max-w-[180px]">
                          {entry.input.text.slice(0, 60)}{entry.input.text.length > 60 ? '...' : ''}
                        </span>
                        <span className="text-[10px] text-emerald-700 shrink-0 ml-1">
                          {new Date(entry.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                      <div className="text-[10px] text-emerald-600 flex items-center gap-2">
                        <Badge variant={entry.result?.environmentalIntent?.category === 'waste' ? 'success' : 'info'} className="text-[8px] px-1 py-0">
                          {entry.result?.environmentalIntent?.category || 'unknown'}
                        </Badge>
                        <span className="text-emerald-700">{entry.result?.actions?.length || 0} actions</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="terminal-panel">
            <div className="terminal-header flex items-center gap-2">
              ...
              <span className="text-emerald-600 text-[10px]">about triage</span>
            </div>
            <div className="terminal-content space-y-2 text-[10px] text-emerald-600/80">
              <div className="flex items-start gap-2">
                <span className="text-emerald-500">&gt;</span>
                <span>Analyzes free-text waste descriptions</span>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-emerald-500">&gt;</span>
                <span>Identifies waste category, sources, and scale</span>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-emerald-500">&gt;</span>
                <span>Generates prioritized action items</span>
              </div>
              <div className="flex items-start gap-2">
                <span className="text-emerald-500">&gt;</span>
                <span>Provides environmental impact projections</span>
              </div>
            </div>
          </div>
        </div>
